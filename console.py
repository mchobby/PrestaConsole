...
class App( BaseApp ):
	...
	def do_order( self, params ):
		...
		def display_order():
			...
			self.output.writeln( 'Carrier      : %s' % self.cachedphelper.carriers.name_from_id( order.id_carrier ) )
		...
		def display_tariff():
			...
			self.output.writeln( '' )
			self.output.writeln( 'Exporting Carrier: %s' % self.cachedphelper.carriers.name_from_id( order.id_carrier ) )
